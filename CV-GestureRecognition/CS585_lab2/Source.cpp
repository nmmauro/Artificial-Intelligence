...
int main()
{
	...
	while (1)
	{
		...
		cv::threshold(con, threshold_output, 150, 255, THRESH_BINARY);
		imshow("threshold", threshold_output);
		
		cv::findContours(threshold_output, contours, hierarchy, 3, 1);

		for (int i = 0; i< contours.size(); i++)
		{
			Scalar color = Scalar(0, 0, 255);
			cv::drawContours(frame, contours, i, color, 1, 8, vector<Vec4i>(), 0, Point());
		}
		imshow("contour drawing", frame);
